...
>主要涉及两个类，Window 和 WindowSpec

>关联知识，SQL种的窗口函数/over等关键字的用法

- Window
  - 为DataFrames提供Window函数的方便的使用方法
  - 实际上就是为WindowsSpec 提供一个快捷使用方法
- WindowSpec
  - 定义分区，排序和帧边界的窗口规范。(通常使用 Window 提供的静态方法来使用这个类)
  - 定义
  ```scala
  class WindowSpec private[sql](
    ...
    frame: WindowFrame)
  ```
- 我们这里直接看WindowSpec就可以了

- 整体分析
  - 参数分析
    - 1. partitionSpec: Seq[Expression]
      - Expression 是交给 Catalyst引擎的分析内容。
    - 2. orderSpec:Seq[SortOrder]
      - SortOrder 用来对 tuple进行sort的的 expression
    - 3. frame:WindowFrame 
      - 代表一个 window框架
      - 对于 groupBy orderBy， 会获取 当前类的 frame参数（Window中创建的 默认 spec 给这个参数赋一个空的站位值）
      - rowsBetween ，rangeBetween 会提供一个 SpecifiedWindowFrame( 一些参数 )
  - Window（Object）里面有一个方法会提供WindowSpec
    - 源码
    ```scala
    ...
    ```
    - 使用Window.xxx()之类的方法时候，会创建一个 WindowSpec实例，然后WindowSpec所有的方法都返回一个新的WindowSpec，实现链式操作
  - 在这个类中，所有的方法都返回一个新的 WindowSpec
