...
- 打开Dataset类的 structure 界面(idea里面)，可以看到其中的各种可用方法，其中最主要的是
  - SQL操作系列
    - 1.join/corssJoin/joinWith
    - 2.sort/sortWithParitions
    - 3.orderBy
    - 4.select/selectExpr /selectUntyped
    - 5.filter - Filters rows using the given SQL expression.(类似于where条件)
    - 6.where
    - 7.groupBy / groupByKey
    - 8.rollup - 使用指定的列为当前数据集创建多维汇总
      - 例子
      ```scala
    ...
    - agg - Aggregates on the entire Dataset without groups.
      ...
      ```
    - limit
    - unionALl/union/unionByName
      - 注意： unionAll/union按照SQL标准，resolves columns by position
      - 注意： unionByName按照schema，resolve columns by name
    - intersect - 交集quivalent to `INTERSECT` in SQL.
    - except - 差集equivalent to `EXCEPT DISTINCT` in SQL.
    - dropDuplicates
    - describe
      - 源码与例子
      ```scala
    ...
    - summary
      ...
      */
      ...
      ```
  - 辅助操作系列 --------------------------------------------------------------------------
    - col/colRegex - Selects column based on the column name and returns it as a [[Column]].
    - as/alias - Returns a new Dataset with an alias set.
    - sample
    - randomSplit/randomSplitAsList - Randomly splits this Dataset with the provided weights.
      - 返回 Array[Dataset[T]]
    - explode  -    @deprecated("use flatMap() or select() with functions.explode() instead", "2.0.0")
      -  functions.explode() => _.XX.split("")
    - withComums - Returns a new Dataset by adding a column or replacing the existing column that has the same name. - ！！！！ 好奇怪，后面测试用法 ！！！
    - drop/dropDuplicatese - Returns a new Dataset with a column dropped. This is a no-op if schema doesn't contain column name.
  - RDD操作系列
    - 不再说明
  - 流操作
    - writeStream
    ...
    ```
  - 其他方法
    - 有一些功能类，适配用的方法等，不在此介绍


    
- Spark 2.3.0 Dataset.scala
...
private[sql] object Dataset {
  def apply[T: Encoder](sparkSession: SparkSession, logicalPlan: LogicalPlan): Dataset[T] = {
    ...
    // 在最开始就确认 encoder 和 schema是匹配的，否则就会报错
  ...
  // object本身相当于 scala中的静态部分，这个是可以直接使用的
  // Dataset.ofRows()
}

[为什么由各Object，为什么Object里面有一个apply方法](../../06.ScalaBasic/20180506-在此学习scala中的object-class.md)
/**
 * A Dataset is a strongly typed collection of domain-specific objects that can be transformed
 * in parallel using functional or relational operations. Each Dataset also has an untyped view
 * called a `DataFrame`, which is a Dataset of [[Row]].

 * dataset 是一个强类型对象，可以使用functional 或者 realational操作进行平行操作。每个dataset都有一个无类型的view叫做dataframe。
 ...
 *  dataset上可执行的操作分类转换操作，行为操作两种。 前者产生新的dataset，后者触发运算，返回结果。
 ...
 * datasets同样是（与RDD一样） lazy模式的，只有action的时候才会产生计算。我们在dataset中生成以logical plan描述处理数据的流程，
 * 每当action启动。 Spark 会优化logical plan生成 physical plan...  （和RDD差不多，不继续翻译了）
 ...
 * 为了 高效的 支持 demain-specific objects , 要求提供一个 Encoder， encoder 给出 domain specific 的类型T，
 * 例如：  给定一个class person，有两个fields name和age。  我们用 encoder 告诉spark 如何在运行时，把person object序列化未
 * 二进制结构。 这个二进制结构 通常占用更少内存，并且在处理数据的时候更加高效。
