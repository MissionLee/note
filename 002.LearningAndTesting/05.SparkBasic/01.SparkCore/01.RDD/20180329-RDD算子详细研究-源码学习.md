...
- Internally, each RDD is characterized by five main properties:
  - A list of partitions - 一个partition列表
  - A function for computing each split - 每个split的运算
  - A list of dependencies on other RDDs - 对于其他rdd的依赖
  - Optionally, a Partitioner for key-value RDDs (e.g. to say that the RDD is hash-partitioned) - 对K-V rdd ，还有一个 Paritioner
  - Optionally, a list of preferred locations to compute each split on (e.g. block locations for an HDFS file) - 可选地，计算每个分割的优选位置的列表（例如HDFS文件的块位置）
 ...
 * through implicit.
```
 对应类型RDD的 所有的操作都通过 implicit 自动获取，
```scala
 /**
...
//  2. 依赖列表
      ...
      //--------------------- 在spark class里面 直接写的代码段会在new的时候被直接执行的！！
  if (classOf[RDD[_]].isAssignableFrom(elementClassTag.runtimeClass)) {
    ...
    // ---- 从逻辑上看，这个方法是用来判断 _sc是未空的
    // ------ RDD 的构造函数要求传入 SparkContest
  ...
  // =======================================================================
  // Methods that should be implemented by subclasses of RDD   子类RDD需要实现的方法！！！！！！！！！！！！！！
  // =======================================================================
  ...
  // =======================================================================
  // Methods and fields available on all RDDs   所有的rdd 都能够获取的方法和field！！！！！！！！！！！！！！
  // =======================================================================
  ...
  def setName(_name: String): this.type = {
    ...
    // 这里的 this.type 是一个技巧，防止 scala子类调用父类方法之后，如果父类返回 this，那么 子类的类型就被转换成了父类类型,这样写
    // 就不会产生这样的问题了
  }
  ...
  private def persist(newLevel: StorageLevel, allowOverride: Boolean): this.type = {
    ...
    //--------stoorageLevel 是 rdd 的一个属性
    //--------    如果rdd已经被设定为 none之外的等级，那么不允许修改
  ...
  //---------------------下面可以看到， cache 其实就是把 storage level 设置未 memory only
  /**
  ...
  final def dependencies: Seq[Dependency[_]] = {
    checkpointRDD.map(r => List(new OneToOneDependency(r))).getOrElse {
      if (dependencies_ == null) {
        dependencies_ = getDependencies  // 这一步 就是  dependencies_ 属性 = 创建rdd时候创建的 deps 
        // 这里我刚开始就犯了错误了，
        // checkpointRDD 本身是一个 Option 类型的，后面的 map  getOrElse 都是  Option的 操作！！！！ 我还以为是RDD操作呢
        ///----------------------------------------------------------------------
        ///----------------------------------------------------------------------
        ///  Option 的 getOrElse ，如果 有值取值，如果没有值，取默认。 这里 默认情况 是 执行 {} 里面的代码块，并且返回 代码块的返回结果！！！！
        ///----------------------------------------------------------------------
        ///  map 的 getOrElse，如果 有对应的key 获取 value 否则 取默认
        ///----------------------------------------------------------------------
        ///----------------------------------------------------------------------
  ...
  ```
> map
> 传入 一个函数 ，每行执行这个函数，返回新的RDD

>[Map算子](./算子测试/Transformations/001.map.md)
...
>[flatMap算子](./算子测试/Transformations/02.flatMap.md)
...
```
>[filter算子](./算子测试/Transformations/03.filter.md)
```
...
// -------------------      下面是 rdd 算子部分的内容  详细看一下
  ...
  //  1- withScoupe ： 执行一个代码块，，所有这个代码块产生的rdd都会在同一个scope里面
  // sc 的 union：   def union[T: ClassTag](first: RDD[T], rest: RDD[T]*): RDD[T] = withScope {union(Seq(first) ++ rest)}
  //      这里可以看到，sc的union拿到来嗯个RDD作为参数，把第一个rdd 放入 Seq里面，把第二个RDD 追加进去，然后调用 另一个union
  //    ！！！！ 注意 ， 另外一个要注意的内容是  rest:RDD[T]*  这是一个变长参数，也就是 rest 实际上的类型 是个 列表
  //     ！！！！！！   这就是 纠结了 一段时间的  Seq(first) ++ rest  因为 Seq 的 ++ 方法，只能传入另一个Seq，刚开始没主意  变长参数这个小细节
  //  -> 看这里使用的另一个union
    def union[T: ClassTag](rdds: Seq[RDD[T]]): RDD[T] = withScope {
      // !!!!!注意 这是 SparkContext 里面的 union！！！！
      // 方法接受一个 rdd Seq
    val partitioners = rdds.flatMap(_.partitioner).toSet
    if (rdds.forall(_.partitioner.isDefined) && partitioners.size == 1) {
      // 判断传入的rdd  如果这些rdd全部分区并且 
      new PartitionerAwareUnionRDD(this, rdds)
    } else {
      
      new UnionRDD(this, rdds)
    }
    // 这里在 构建新的rdd的时候 ，都传入了 this
  }
  ...
  def sortBy[K](
    ...
    this.keyBy[K](f)
        ...
        .values
        // 从这里面 ，正好分析以下 scala 的一些语法细节
        
  }
```
[从SortBy分析学习scala语法](./20180330-从sortBy学习-分析scala的语法.md)
```scala
    //后面 我看看看 具体做了什么  ---> 这里，sortBy 实际上是强化的sortByKey，我先把sortByKey 弄明白
this.keyBy[K](f)
        .sortByKey(ascending, numPartitions)
        .values
   // 这里要插入两部分内容
```
> [implicit 在RDD源码中的使用](./20180331-RDD源码中的implicit.md)

> [OrderedRDDFunction源码学习-sortByKey](./20180331-OrderedRDDFunction源码分析.md)
