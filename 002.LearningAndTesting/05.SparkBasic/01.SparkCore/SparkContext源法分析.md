...
- 分析过程
  - 1.先看Object里面有哪些初始工作
    ...
    - 部分方法
  - 2.看创建的时候的初始化工作
  - 3.由SparkContext初始化创建的各个类开始，像外扩展

- object SparkContext
  - 一个全局的锁 new Object
  - 一个全局的 activeContext ： AtomicReference[SparkContext]
  - 一个存储仅部分构建的SparkContext的 contextBeing
  Constructed : Option[SparkConetxt]
  - 一个验证 SC 唯一性的方法： 确保当前没有其他SC正在运行
  - 一个标准的 getOrCreate 方法
  - 其他一些 关于 sparkContext的方法
  - 初始化一些量 
  - 定义了一个 arrayToArrayWritable 方法
- class SparkContext [只分析初始化部分]
  - 首先是很多私有变量
  ```scala
  ...
  ```
  - 一些变量的Accessor（私有，共有，spark内私有都存在） 和一些 共有变量 - [仅贴出来其中几个]
  ```scala
  ...
  ```
  - 一个初始化代码块 ```try{}catch{}```
    - 这里需要知道一些 scala类加载顺序的知识，可以看 scala目录下的文档
    - 获取配置参数，做简单验证
    - 获取用户提交的jar包
    - 获取用户提交的文件
    - 初始化/启动/创建
      - ListenBus
      - ApptatusStore
      - SparkEnv (ListenBus会传给SparkEnv) / 下面很多内容在 SparkEnv中，不一一指出了
      - StatusTracker
      - progressBar
      - ui相关设置
      - hadoopConfiguration
      - executorMemory
      - heartbeatReceiver
      - schedulerBackend & taskScheduler (这两个根据模式不同，成对设置)
      - dagScheduler
      - 启动 taskScheduler
      - 从 taskScheduler 获取 appId/appAttempId
      - 启动 blickManager （需要 appId）
      - 启动 metricsSystem
      - 启动一些辅助内容
      - 启动 ListenerBus
      - 把模块之间需要互相注册的内容注册了
...
class SparkContext(config: SparkConf) extends Logging {
  ...
  try {  // 这里 的 try  开始 真正的 初始化方法！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！
         // ！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！
    _conf = config.clone()
    _conf.validateSettings()

    if (!_conf.contains("spark.master")) { //判断 master 是否设置
      throw new SparkException("A master URL must be set in your configuration")
    }
    if (!_conf.contains("spark.app.name")) { // 判断appname 是否设置
    ...
    if (master == "yarn" && deployMode == "cluster" && !_conf.contains("spark.yarn.app.id")) {
        // 必须用 spark-submit  运行 Spark on Yarn 任务
    ...
    _jars = Utils.getUserJars(_conf) // 获取用户 jars
    _files = _conf.getOption("spark.files").map(_.split(",")).map(_.filter(_.nonEmpty))
      .toSeq.flatten // 获取 spark files
    ...
    _listenerBus = new LiveListenerBus(_conf) // 创建 LiveListenerBus  Asynchronously（异步的） passes SparkListenerEvents to registered SparkListeners.
    ...
    _env = createSparkEnv(_conf, isLocal, listenerBus) // ！！ 重点内容值一 createSparkEnv
    SparkEnv.set(_env)
```
[SparkEnv简单分析](./SparkEnv分析.md)
```scala
    ...
    _statusTracker = new SparkStatusTracker(this, _statusStore) // 监控 job 和 stage 状态的
    // 下面两个和 Spark ui 有关的东西
    _progressBar =
    ...
    _hadoopConfiguration = SparkHadoopUtil.get.newConfiguration(_conf) // hadoop 内部自动支持
    ...
    // 我们需要在 createTaskScheduler之前 注册 HeartbeatReceiver，因为Executor构造的时候需要
    _heartbeatReceiver = env.rpcEnv.setupEndpoint(
      HeartbeatReceiver.ENDPOINT_NAME, new HeartbeatReceiver(this))

    // Create and start the scheduler   创建 BackEnd 和 TaskScheduler DagScheduler
    // ！！！
    ...
    // constructor
    _taskScheduler.start() //启动 TaskScheduler
    // 下面这两个 值的初始化就很神奇了， 为什么一个用 内部变量 一个用方法 来获得
    //
    // 我的想法 ： _applicationId 是不变的
    //            _applicationAttempId 是变化的 -> 所以没次获取都要调用方法，获取新的值
    //              以上猜想没验证成功
...
```
##  metricsSystem    ！  需要看一下相关内容  https://www.jianshu.com/p/5506cd264f4d
...
```scala
   //根据需求 启动 logger
    ...
    // ListenerBus  Environment
    setupAndStartListenerBus() // 注册listeners 启动 listener bus 后面方法里面有详细内容
    postEnvironmentUpdate() // 发起一次 环境变更事件（task scheduler is ready'） 后面有详细内容
    postApplicationStart() // application start

    // Post init  初始化内容
    _taskScheduler.postStartHook()  // taskScheduler 里面 分析
    _env.metricsSystem.registerSource(_dagScheduler.metricsSource)  // metricsSystem 里面分析
...
object SparkContext extends Logging {
   ...
   * Object里面要创建null的，真正的创建，会更新activeContext
   */
```
# ！！！ AtomicReference
```scala
   ...
   * constructed and cases where another constructor threw an exception.
   *
   * 再 SparkContext 调用构造器的时候确保 当前没有 SparkContext在运行
   */
   ...
   *
   * 在SparkContext 构造器最后步骤执行，确保没有构造器与当前构造器竞争，并先一步启动了
   */
  ...
  // 隐式转换  
  // ArrayWirtable 是 hadoop.io 中的 A Writable for arrays containing instances of a class. 
...
```scala
   ...
   *
   * 创建 task scheduler 的方法
   * 返回一个 二元组 （scheduler backend ， task scheduler）
   */
